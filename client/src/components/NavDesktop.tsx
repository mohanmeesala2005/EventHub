...
const NavDesktop: React.FC<Props> = ({ user, profileOpen, setProfileOpen, handleLogout }) => {
  return (
    <div className="hidden md:flex items-center space-x-8">
      ...
      <Link to={user?.role === 'admin' ? '/admin/dashboard' : '/dashboard'} className={linkClass}>
        Dashboard
        <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-blue-400 transition-all group-hover:w-full"></span>
      </Link>
      ...
      {user ? (
        <>
          ...
          <div className="relative">
            <button
              onClick={() => setProfileOpen?.((prev: any) => !prev)}
