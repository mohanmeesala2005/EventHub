...
const Navbar: React.FC = () => {
  ...
  return (
    <header className="sticky top-0 w-full z-50 bg-slate-900 backdrop-blur-md border-b border-slate-700/50 shadow-lg">
      <nav className="px-4 py-3 flex items-center justify-between max-w-7xl mx-auto">
        ...
        <div className="flex items-center space-x-3">
          <Link to="/" className="flex items-center space-x-3 group">
            <img src="/eventHub_logo.png" alt="EventHub Logo" className="w-8 h-8 rounded-lg" />
          </Link>
        </div>

        <NavDesktop user={user} profileOpen={profileOpen} setProfileOpen={setProfileOpen} handleLogout={handleLogout} />
      ...
      {menuOpen && <NavMobile user={user} onClose={() => setMenuOpen(false)} handleLogout={handleLogout} />}
    </header>
  );
};
